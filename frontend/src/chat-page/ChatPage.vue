...
// panel's App.vue, minus the panel-only chrome (close/fullscreen/resize).
onMounted(async () => {
	await loadInitial();
	// The page has no "open" toggle to gate this on (unlike the panel) — it's
	// always open, so pick up the last-used session on every load.
...
<template>
	<div ref="page" class="flow-panel flow-chat-page flex h-full text-ink-gray-9">
		<Sidebar />
		<div class="relative flex min-w-0 flex-1 flex-col bg-surface-white">
