		...
		for (const m of doc.messages || []) {
			if (m.role === "user") {
				...
				if (currentAssistantMsg) {
					const tc = currentAssistantMsg.parts.find(
						(p) => p.type === "tool" && p.id === m.tool_call_id
					);
...
async function send() {
	...
	try {
		const resp = await fetch(`/api/method/flow.api.start_run`, {
			method: "POST",
			headers: {
...
async function resume(answers) {
	...
	try {
		const resp = await fetch(`/api/method/flow.api.resume_run`, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				"X-Frappe-CSRF-Token": frappe.csrf_token,
	...
	<div class="chat-sidebar">
		...
		<div class="cs-header">
			<div class="cs-header-row">
				...
				<div class="dd-root">
					<button
						class="fui-btn ghost sm icon"
						:title="__('Previous sessions')"
					...
					<template v-if="sessionsOpen">
						...
						<div class="sessions-menu right">
							<div class="sessions-head">
								<span class="sessions-head-title">{{
									__("Recent sessions")
								}}</span>
								<button
							...
							<div class="sessions-list thin-scroll">
								...
								<div v-if="!recentSessions.length" class="sessions-empty">
				...
				<button class="fui-btn ghost sm icon" :title="__('New chat')" @click="newChat">
					<Icon name="plus" :size="14" :strokeWidth="2" />
				</button>
				<button
					class="fui-btn ghost sm icon"
					:title="__('Close (Ctrl+M)')"
					@click="onClose"
				>
					<Icon name="x" :size="14" :strokeWidth="2" />
				</button>
			...
			<div v-if="needsSetup" class="cs-setup">
				...
				<ul class="setup-steps">
					...
					<li>
						{{
							__(
								"Enable Server Scripts in your site config so the assistant can run code."
							)
						}}
					</li>
				</ul>
								...
								>
									...
									>
										...
										<span class="tc-status">
											<span
												class="dot"
												:class="part.result === null ? 'running' : 'done'"
											/>
								...
								<div v-for="q in msg.questions" :key="q.key" class="confirm">
									...
									<div v-if="!q._showOther" class="confirm-actions">
										<button
											...
											class="fui-btn sm"
											:class="
												opt === 'Approve' ? 'confirm-approve' : 'ghost'
											"
				...
				<div class="composer-controls">
					...
					<div class="dd-root">
						<button
							...
							:title="__('Agent')"
							@click="!locked && (agentOpen = !agentOpen)"
						>
					...
					<button
						class="send-btn"
						:disabled="!inputText.trim() || sending || paused || needsSetup"
						:title="__('Send')"
						@click="send"
					>
