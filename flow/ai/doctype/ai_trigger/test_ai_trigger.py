...
class TestAITriggerFiring(IntegrationTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		sync_builtin_tools()
	...
	def test_dispatch_scheduled_fires_when_due(self):
		...
		frappe.db.set_value("AI Trigger", trig.name, "last_fired_at", two_hours_ago, update_modified=False)
		...
		last = frappe.db.get_value("AI Trigger", trig.name, "last_fired_at")
	...
	def test_dispatch_scheduled_skips_recently_fired(self):
		...
		trig = self._scheduled("* * * * *")
		frappe.db.set_value(
			"AI Trigger", trig.name, "last_fired_at", frappe.utils.now_datetime(), update_modified=False
		)
