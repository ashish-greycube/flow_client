...
is stored (as JSON) in the "codex" Flow Provider's api_key field and refreshed here
in place, so no API key or per-token billing is involved. The backend only speaks
the Responses SSE protocol, which litellm cannot, so this module adapts each stream
into the chat-completions chunk shape flow.lib.model already reads: Model.chat()
hands any model whose id starts with "codex/" to complete() below and everything
downstream (streaming, tool calls, usage) stays provider-blind.
"""
...
# Named so is_retryable-style checks can recognise them by exception class name.
...
def complete(
	...
	*,
	...
	`params` is accepted for interface parity and deliberately unsent: the codex
	backend 400s on temperature AND on max_output_tokens, and budgets output itself."""
...
class ChunkAssembler:
	"""Folds Responses SSE events into the chunk shape flow.lib.model._consume_stream
	reads. Tool-call arguments stream as deltas; the done event only tops up
	whatever the deltas had not sent, so nothing arrives twice."""
...
def runtime_auth(provider: str) -> tuple[str, str]:
	...
	if not cred.get("account_id"):
		raise CodexError(
			"The ChatGPT credential is missing its account id. Connect with ChatGPT again on the Flow Provider."
		)
...
def stored_credential(provider: str) -> dict:
	...
	raw = get_decrypted_password("Flow Provider", provider, "api_key", raise_exception=False)
	if not raw:
		raise CodexError("No ChatGPT credential is saved. Connect with ChatGPT on the Flow Provider.")
...
def save_credential(provider: str, cred: dict) -> None:
	...
	set_encrypted_password("Flow Provider", provider, json.dumps(cred), "api_key")
...
def parse_credential(raw: str) -> dict:
	...
	if not isinstance(data, dict):
		raise CodexError(
			"That doesn't look like a ChatGPT credential. Connect with ChatGPT, or paste the full contents of ~/.codex/auth.json."
		)
	...
	if not data.get("refresh"):
		raise CodexError("The credential has no refresh token. Connect with ChatGPT again.")
	return data
...
def refresh_credential(cred: dict) -> dict:
	...
	if resp.status_code >= 400:
		raise CodexError("The ChatGPT sign-in has expired. Connect with ChatGPT again.")
	data = resp.json()
...
def log(message: str) -> None:
	...
	logger = frappe.logger("flow.lib.codex")
