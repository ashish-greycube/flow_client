...
"""Browser sign-in for the ChatGPT (Codex) pseudo-provider.
...
from flow.lib.codex import CLIENT_ID, ORIGINATOR, TOKEN_URL, CodexError, account_id_from_tokens
...
def finish(redirect_url: str) -> dict:
	...
	login = load_login(state)
	if not login:
		raise CodexError("This sign-in attempt has expired. Click Connect with ChatGPT and try again.")
...
def install_provider(credential: dict) -> None:
	if frappe.db.exists("Flow Provider", PROVIDER_NAME):
		doc = frappe.get_doc("Flow Provider", PROVIDER_NAME)
	else:
		doc = frappe.get_doc({"doctype": "Flow Provider", "provider": PROVIDER_NAME})
	doc.enabled = 1
	doc.api_key = json.dumps(credential)
...
def exchange_code(code: str, verifier: str) -> dict:
	...
	if resp.status_code >= 400:
		raise CodexError("ChatGPT rejected the sign-in code. Click Connect with ChatGPT and try again.")
	tokens = resp.json()
...
def login_key(state: str) -> str:
	# prefixed so a bench running both flow and builder doesn't share cache keys
	return f"flow_codex_oauth:{state}"
...
def start_listener(site: str) -> bool:
	...
	with listener_lock:
		...
		try:
			...
			return False  # someone else holds the port: the paste path still works
		server.site = site
...
class CallbackHandler(BaseHTTPRequestHandler):
	def do_GET(self):
		...
		if outcome is None:
			self.respond(400, "This sign-in attempt is stale. Return to Flow and try again.")
			return
...
def complete_captured(site: str, state: str, code: str) -> tuple[bool, str] | None:
	...
	try:
		...
		try:
			...
			return True, "Connected. You can close this tab and return to Flow."
		except Exception as e:
			save_login(state, {"status": "failed", "message": str(e)})
			return False, "Sign-in failed. Return to Flow and try again."
	finally:
		frappe.destroy()


PAGE = """<!doctype html>
<html><head><meta charset="utf-8"><title>Flow</title></head>
